...
# local cache directory (clean these paths up?)
local_cd <- "/shared/wiktionary"
processed_wiktionary <- readRDS(file.path(local_cd, "wikimorphemes.rds"))

wp_path <- "/shared/BERT_checkpoints/bert_base_uncased/vocab.txt"
# {wordpiece} formats the vocab as a named integer vector, where the name is 
# the token and the value is the token id.
wp_vocab <- wordpiece::load_or_retrieve_vocab(wp_path)
...
Note that only about 5% of the morpheme tokens (out of about 190K) are needed to 
...
corpus_coverage_large %>% 
  filter(!covered) %>% 
  arrange(desc(word_count)) %>% head(10)
```
...
ss_url <- "https://docs.google.com/spreadsheets/d/1iVUuRPVfNMhfSs5mRglV9AgsJcDHABQKv4pawfXJvoE/"

# already authorized
googlesheets4::write_sheet(words, ss_url)
# manual check, add column "is_ok"
checked_words <- googlesheets4::read_sheet(ss_url, sheet = "checked")
...
Randomly pick some words of at least length 4 without a breakdown...
...
# manual check, add column "is_ok"
checked_unbroken_words <- googlesheets4::read_sheet(ss_url, 
                                                    sheet = "unbroken_check")
...
Also, for words that are not in the lookup, we can test the fall-through by evaluating a sample?
