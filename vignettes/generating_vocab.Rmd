...
words_with_max_token_ranks <- dplyr::left_join(
  ...
  dplyr::filter(!is.na(.data$max_rank))

# Count how many total words are covered by tokens up to some rank:
words_vs_tokens <- words_with_max_token_ranks %>% 
  dplyr::group_by(.data$max_rank) %>% 
  dplyr::summarize(n_words = dplyr::n_distinct(.data$word)) %>% 
  dplyr::arrange(.data$max_rank) %>% 
  dplyr::mutate(n_words = cumsum(.data$n_words))
```
...
# plot!
words_vs_tokens %>% 
  dplyr::mutate(frac_words = .data$n_words/max(.data$n_words)) %>% 
  dplyr::mutate(frac_tokens = .data$max_rank/max(.data$max_rank)) %>% 
  ggplot2::ggplot(ggplot2::aes(x = .data$frac_tokens, y = .data$frac_words)) + 
  ggplot2::geom_point() 
...
words_vs_tokens_weighted <- weighted_tokens_and_words %>% 
  dplyr::group_by(.data$max_rank) %>% 
  dplyr::summarize(n_words = sum(.data$word_count)) %>% 
  dplyr::arrange(.data$max_rank) %>% 
  dplyr::mutate(n_words = cumsum(.data$n_words))
...
corpus_coverage_small <- dplyr::left_join(
  ...
  dplyr::mutate(covered_lookup = !is.na(.data$tokenization)) %>% 
  # not every word in the vocab is in the lookup; check vocab too
  dplyr::mutate(covered_vocab = .data$word %in% vandl_small$vocab) %>% 
  dplyr::mutate(covered = .data$covered_lookup | .data$covered_vocab) %>% 
  dplyr::mutate(covered_weighted = .data$covered*.data$word_count) 

corpus_coverage_small %>% 
  dplyr::summarize(sum(.data$covered_weighted)/sum(.data$word_count))
```

The "small" vocabulary covers 95% of words from the corpus, weighted by usage. 
...
corpus_coverage_large <- dplyr::left_join(
  ...
  dplyr::mutate(covered_lookup = !is.na(.data$tokenization)) %>% 
  # not every word in the vocab is in the lookup; check vocab too
  dplyr::mutate(covered_vocab = .data$word %in% vandl_large$vocab) %>% 
  dplyr::mutate(covered = .data$covered_lookup | .data$covered_vocab) %>% 
  dplyr::mutate(covered_weighted = .data$covered*.data$word_count) 
...
The "large" vocabulary covers 96.5% of words, 1.5% more words (weighted) than the "small" vocabulary. 
...
Our process (as of August 2021) has a hole: abbreviations such as "km" and "cm" really should be in the vocabulary, but they are missing due to not having an "English" section in Wiktionary.
We'll address this in a future update.

Other categories of uncovered words include some proper nouns, words with diacritical marks, and partial contractions (like "didn").

The word "los" in this list raises an interesting question for future consideration: should a small set of common words in languages besides English be included? After all, a significant number of tokens in both {wordpiece} and {morphemepiece} are currently used for Asian language characters.
